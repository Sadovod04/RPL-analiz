...
STRINGS = {
    "ru": {
        ...
        "src_ffspb": "ФФ СПб (дети 2012–2015)",
        "src_help": "Трансфермаркт — игроки академий с карьерой (модель). ФФ СПб — дети, у которых карьеры ещё нет (оценка по эвристике).",
        "col_method": "Метод",
        "method_model": "модель",
        ...
        "youth_note": "Данные ФФ СПб (детские турниры). Эти дети теперь и в общем списке «Перспективные» (фильтр «Источник» → ФФ СПб) со своей оценкой. У них ещё нет карьеры для модели — оценка эвристическая: голы за игру, объём игр, сила клуба.",
    ...
    "en": {
        ...
        "src_ffspb": "SPb FF (kids 2012–2015)",
        "src_help": "Transfermarkt — academy players with a career (model). SPb FF — kids with no career yet (heuristic score).",
        "col_method": "Method",
        "method_model": "model",
        ...
        "youth_note": "SPb FF data (kids' tournaments). These kids are now also in the main 'Prospects' list (Source filter → SPb FF) with their own score. No career yet for the model — the score is heuristic: goals per game, games played, club strength.",
